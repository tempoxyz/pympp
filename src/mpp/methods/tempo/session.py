...
from mpp import Challenge, Credential, MemoryStore, Receipt, Store
...
if TYPE_CHECKING:
    ...
    from mpp.runtime import AsyncHttpResponseContext, SyncHttpResponseContext
...
class TempoSessionMethod:
    ...
    async def _open(self, request: _Request, deposit: int, cumulative: int) -> dict[str, Any]:
        ...
        transaction, unsigned = await self._transaction(
            request, call, nonce_key=_session_nonce_key(salt)
        )
        try:
    ...
    async def _transaction(
        ...
        nonce = 0 if request.fee_payer else await self._lane_nonce(nonce_key)
        priority = gas_price
        ...
        tx = TempoTransaction.create(
            ...
            nonce=nonce,
            nonce_key=EXPIRING_NONCE_KEY if request.fee_payer else nonce_key,
    ...
    async def _lane_nonce(self, nonce_key: int) -> int:
        data = _call(
            ...
            ["address", "uint256"],
            [self.account.address, nonce_key],
        )
        result = await _rpc_call(
            self.rpc_url,
            "eth_call",
            [{"to": NONCE_PRECOMPILE, "data": "0x" + data.hex()}, "pending"],
        )
        try:
            nonce = int(result, 16)
        except (TypeError, ValueError) as error:
            raise TransactionError("Tempo nonce RPC returned invalid data") from error
        if nonce > MAX_UINT64:
            raise TransactionError("Tempo nonce RPC returned a value outside uint64 bounds")
        return nonce
...
def _session_nonce_key(salt: str) -> int:
    """Derive one reusable, non-reserved 2D nonce lane per channel."""
    value = bytes.fromhex(_bytes32(salt, "descriptor.salt")[2:])
...
def _voucher_signature(
    ...
    return "0x" + account.sign_hash(_keccak(b"\x19\x01" + domain + voucher)).hex()
