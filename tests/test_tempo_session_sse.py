...
def test_sync_wrapper_preserves_response_identity_and_elapsed() -> None:
    stream = TrackingSyncStream(b"data: application\n\n")
    with httpx.Client(
        transport=httpx.MockTransport(lambda _: httpx.Response(200, stream=stream))
    ) as client:
        response = client.send(client.build_request("GET", "https://example.test"), stream=True)
        wrapped = wrap_sync_sse_response(
            response,
            on_need_voucher=lambda _: None,
            on_receipt=lambda _: None,
        ...
        assert wrapped.read() == b"data: application\n\n"
        assert wrapped.elapsed.total_seconds() >= 0
...
@pytest.mark.asyncio
async def test_async_wrapper_preserves_response_identity_and_elapsed() -> None:
    stream = TrackingAsyncStream(b"data: application\n\n")
    ...
    async with httpx.AsyncClient(transport=httpx.MockTransport(handle)) as client:
        ...
        wrapped = wrap_async_sse_response(
            response,
            on_need_voucher=AsyncMock(),
            on_receipt=AsyncMock(),
        )

        assert wrapped is response
        assert await wrapped.aread() == b"data: application\n\n"
        assert wrapped.elapsed.total_seconds() >= 0
